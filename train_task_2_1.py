...
class CoOpTrainer:
    ...
    # def _compute_detection_loss(self, pred_boxes, pred_logits, gt_boxes, image_shape, class_token_embs):
    #     """
    #     Simple + correct GDINO-compatible loss.
    #     Supervision uses ONLY the tumor token logits.
    #     """

    #     h, w = image_shape

    #     # ----------------------------
    #     # 1. Convert predicted boxes
    #     # ----------------------------
    #     pred_xyxy = box_convert(pred_boxes, in_fmt="cxcywh", out_fmt="xyxy")

    #     # ----------------------------
    #     # 2. Select tumor-token logits
    #     # ----------------------------
    #     # # text_tokens = [context tokens..., "a", "malignant", "tumor"]
    #     # # tumor_token_index = -1    # last token (you can detect via tokenizer too)

    #     # # tumor_logits = pred_logits[:, tumor_token_index]  # shape [N]
    #     # # print(f"class_token_embs : {class_token_embs}")
    #     # mapped_class = self.model.feat_map(class_token_embs.to(self.device))   # [T, 256]
    #     # # print(f"mapped_class : {mapped_class}")
    #     # print(f"mapped_class shape : {mapped_class.shape}")
    #     # tumor_vec = mapped_class.mean(dim=0)                                   # [256]
    #     # # print(f"tumor_vec : {tumor_vec}")
    #     # print(f"tumor_vec shape : {tumor_vec.shape}")
    #     # # -------------------------------------------------
    #     # # 1. Compute tumor score = cosine sim(query, tumor_vec)
    #     # # -------------------------------------------------
    #     # # pred_logits IS the query embedding from DINO
    #     # query_features = pred_logits                                            # [N, 256]

    #     # # print(f"query_features shape : {query_features.shape}")
    #     # # print(f"query_features : {query_features}")
    #     # # print(f"tumor_vec shape : {tumor_vec.unsqueeze(0).shape}")  
    #     # # print(f"tumor_vec : {tumor_vec.unsqueeze(0)}")
    #     # tumor_logits = F.cosine_similarity(
    #     #     query_features,
    #     #     tumor_vec.unsqueeze(0),
    #     #     dim=-1,
    #     # )
    #     # print(f"tumor_logits shape: {tumor_logits.shape}")
    #     # # print(f"tumor_logits : {tumor_logits}")



    #     # ----------------------------
    #     # 3. No GT → treat entire image as benign background
    #     # ----------------------------
    #     if len(gt_boxes) == 0:
    #         # target = torch.zeros_like(tumor_logits)
    #         # cls_loss = F.binary_cross_entropy_with_logits(tumor_logits, target)
    #         cls_loss = torch.tensor(0.0, device=self.device)
    #         return cls_loss, {"ap": 0.0, "num_gt": 0, "num_pos": 0, 'cls_loss': cls_loss.item(), 'giou_loss': 0.0}

    #     # ----------------------------
    #     # 4. Normalize GT boxes
    #     # ----------------------------
    #     gt = torch.tensor(gt_boxes, dtype=torch.float32, device=self.device)
    #     gt_norm = gt.clone()
    #     gt_norm[:, [0,2]] /= w
    #     gt_norm[:, [1,3]] /= h
    ...
    #     # 5. IoU Matching
    #     # ----------------------------
    #     ious = box_iou(pred_xyxy, gt_norm)
    #     max_iou, matched_idx = ious.max(dim=1)

    #     pos_mask = max_iou >= 0.5
    #     labels = pos_mask.float()   # 1 for tumor, 0 for background

    #     # ----------------------------
    #     # 6. Classification loss (tumor-token only)
    #     # ----------------------------
    #     # print(f"len_gt: {len(gt_boxes)}, len_pred: {len(pred_boxes)}, tumor_logits: {tumor_logits.shape}, labels: {labels.shape}")
    #     # if torch.isnan(tumor_logits).any():
    #     #     print("NAN IN tumor_logits")
    #     # if torch.isinf(tumor_logits).any():
    #     #     print("INF IN tumor_logits")
    ...
    #     # print("logit stats:", tumor_logits.min().item(), tumor_logits.max().item())
    #     # print("label stats:", labels.min().item(), labels.max().item())
    #     # cls_loss = F.binary_cross_entropy_with_logits(tumor_logits, labels)
    #     cls_loss = torch.tensor(0.0, device=self.device)

    #     # ----------------------------
    #     # 7. Box regression (L1 on positives only)
    #     # ----------------------------
    #     if pos_mask.sum() > 0:
    #         pred_pos = pred_xyxy[pos_mask]
    #         gt_pos   = gt_norm[matched_idx[pos_mask]]
    #         box_loss = F.l1_loss(pred_pos, gt_pos)
    #     else:
    #         box_loss = torch.tensor(0.0, device=self.device)

    #     total = cls_loss + 5.0 * box_loss

    #     # print(f"cls_loss: {cls_loss.item()}, box_loss: {box_loss.item()}, total: {total.item()}")
    ...
    #         'giou_loss': box_loss.item() if torch.is_tensor(box_loss) and box_loss.requires_grad else 0.0, 
    #     }

            
    #         # metrics = {
    #         #     'ap': ap,
    #         #     'num_gt': len(gt_boxes_array),
    #         #     'num_pred_filtered': score_stats['above_01'],
    #         #     'max_score': score_stats['tumor_max'],
    #         #     'num_pos': pos_mask.sum().item(),
    ...
    def train(self, train_path, train_csv="train.csv",
              ...
              save_path="trained_coop_context.pt", num_examples=None, batch_size=16):
