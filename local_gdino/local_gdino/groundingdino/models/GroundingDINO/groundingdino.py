...
class GroundingDINO(nn.Module):
    ...
    # aastha: extra helper for CoOp:
    def get_text_embeddings(self, caption):
        """
        Compute BERT last_hidden_state embeddings for a caption string.
        This bypasses special-token logic used by GroundingDINO during training.
        Returns: [T, D_bert] or [B, T, D_bert]
        """
        ...
        # Basic tokenization only — NO special GroundingDINO masking
        tokenized = tokenizer(
            captions,
            padding="longest",
            return_tensors="pt"
        ...
        if single:
            return hidden[0]   # [T, D_bert]

        return hidden      # [B, T, D_bert]
