...
class CoCoOpPromptLearner(torch.nn.Module):
    ...
    def forward(self, image_embedding: torch.Tensor):
        ...
        # meta-net projection
        proj = self.meta_net(image_embedding)     # [D]

        # add to every context vector
        context = self.context + proj.unsqueeze(0)  # [K, D]
        return context
...
class CoCoOpTrainer:
    ...
    def _init_coop(self, init_prompt: str):
        ...
        learner = CoCoOpPromptLearner(self.k, mean_vec.shape[0], mean_vec).to(self.device)
    ...
    def train(self, train_path, train_csv="train.csv",
        ...
        for ep in range(1, epochs + 1):
            ...
            for batch_idx in pbar:
                ...
                with torch.no_grad():
                    feats_all = self.model.backbone(images_nested_tensor) # returns feats, masks
                    feats = feats_all[0] # feats: list of NestedTensor
                    # print(f"feats length: {len(feats)}")
                    # choose the final feature level (usually the most semantic)
                    last_level = feats[-1]     
                    # print(f"last_level shape: {last_level.shape}") # NestedTensor
        ...
        torch.save(learner.meta_net.detach().cpu(), save_path.replace(".pt", "_meta_net.pt"))
...
def main(dataset_path=None, csv_path=None, save_path=None, num_examples=None):
    trainer = CoCoOpTrainer(device=DEVICE, k_context=8)
    ...
    save_path = save_path if save_path is not None else PATHS["task_2_2_save"] + train_path.replace("/", "_").replace(".", "") + ".pt"
...
if __name__ == "__main__":
    ...
    main(dataset_path=PATHS["train_A"], csv_path="train.csv", save_path=PATHS["task_2_2_save"] + PATHS["train_A"].replace("/", "_").replace(".", "") + ".pt", num_examples=num_examples)
    main(dataset_path=PATHS["train_B"], csv_path="train_updated.csv", save_path=PATHS["task_2_2_save"] + PATHS["train_B"].replace("/", "_").replace(".", "") + ".pt", num_examples=num_examples)
    main(dataset_path=PATHS["train_C"], csv_path="train.csv", save_path=PATHS["task_2_2_save"] + PATHS["train_C"].replace("/", "_").replace(".", "") + ".pt", num_examples=num_examples)