...
class GroundingDinoModel(GroundingDinoPreTrainedModel):
    ...
    def forward(
        ...
        return_dict=None,
        mode: str = "vanilla",
        ctx_vectors: Optional[torch.FloatTensor] = None,
    ):
        r"""
        ...
        # if mode is not "coop" or ctx_vectors is None:
        #     # print("using text features from text backbone for mode: ", mode)
        #     text_features = self.text_projection(text_features)
        # elif mode is "coop" and ctx_vectors is not None:
        #     # print("using text features from coop for mode: ", mode)
        #     text_features[:, 1:1+ctx_vectors.shape[0]] = ctx_vectors
        #     text_features = self.text_projection(text_features)

        # ===== CoOp Context Injection =====
        # modify embeddings not features lol
        if mode == "coop" and ctx_vectors is not None:
            # Get embeddings from BERT embedding layer
            text_embeddings = self.text_backbone.embeddings(
                input_ids=input_ids,
            ...
            # Find [PAD] token positions and replace with context vectors
            pad_token_id = 0  # or get from tokenizer
            batch_size = input_ids.shape[0]
            K = ctx_vectors.shape[0]
            bos_token_id = 101
            eos_token_id = 102

            ctx_vectors = ctx_vectors.to(text_embeddings.device)

            # Add context to positions 1 through 11 (skipping [CLS] at 0)
            text_embeddings[:, 1:1+K, :] += ctx_vectors.unsqueeze(0)  # Broadcasting across batch dimension
            
            # print("ctx_vectors: ", ctx_vectors.shape)
            # print(f"text_embeddings.device: {text_embeddings.device}")
            # print(f"ctx_vectors.device: {ctx_vectors.device}")
            # for i in range(batch_size):
            #     # pad_positions = (input_ids[i] == pad_token_id).nonzero(as_tuple=True)[0]
            #     pad_positions = ((input_ids[i] != bos_token_id) & (input_ids[i] != eos_token_id)).nonzero(as_tuple=True)[0]
            #     if len(pad_positions) >= K:
            #         for j in range(K):
            #             # text_embeddings[i, pad_positions[j]] = ctx_vectors[j]
            #             text_embeddings[i, pad_positions[j]] += ctx_vectors[j]
            # print("pad_positions: ", pad_positions)
            # Get extended attention mask
            extended_attention_mask = self.text_backbone.get_extended_attention_mask(
                attention_mask, 
                input_ids.shape
            )
            
            # Forward through BERT encoder with modified embeddings
            text_outputs = self.text_backbone.encoder(
                text_embeddings,
                attention_mask=extended_attention_mask,
            )
            text_features = text_outputs.last_hidden_state
        else:
            # Vanilla mode - use normal text backbone
            text_outputs = self.text_backbone(
                input_ids, text_self_attention_masks, token_type_ids, position_ids, return_dict=return_dict
            )
            text_features = text_outputs.last_hidden_state if return_dict else text_outputs[0]
        
        # Project text features to d_model dimension
        text_features = self.text_projection(text_features)
        # ===== End CoOp Injection =====
...
class GroundingDinoForObjectDetection(GroundingDinoPreTrainedModel):
    ...
    def __init__(self, config: GroundingDinoConfig):
        print("using local transformers")
        super().__init__(config)
    ...
    def forward(
        ...
        #         if label["class_labels"].shape[0] > 0:
        #             print(f"Positive example: avg of positive logits: ", logits[i, :, 1:5].mean())
        #         else:
        #             print(f"Negative example: avg of positive logits: ", logits[i, :, 1:5].mean())
